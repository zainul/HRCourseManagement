...
module.exports = {
  ...
  get_kendo_as_subgrid_format:function(req,res){
    Training.query("SELECT t.name,t.code,td.name as 'directory' FROM trainings t JOIN training_directories td " +
      "ON t.directory=td.id  WHERE user_level='"+req.param('userlevel')+"' ", function(err, results) {
      if (err) return res.serverError(err);
      return res.ok(results);
    });
  },
